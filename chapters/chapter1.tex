...
\item Rigid Lexical Matching: Many platforms rely on exact keyword matching. This approach is limited in its ability to handle synonyms, technical jargon, and relationships between concepts. As a result, users may miss relevant listings because their search terms do not precisely match the employer's vocabulary \cite{baeza2011modern}.
...
\item RQ3: How much does a hybrid retrieval strategy---one that combines lexical and semantic (vector) search---improve the relevance and recall of internship discovery compared to traditional keyword search alone?
...
\item A Hybrid Search Infrastructure: The thesis implements a retrieval system using an in-memory vector index and lexical search to support hybrid search functionality.
